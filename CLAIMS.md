...
| **The shipped prober's own store now holds 12,403 probed mainnet agents. 1,388 of them declare a machine-callable protocol; 19 answer on one — 1.4% of those that claim to be callable, 0.15% of all probed.** | `REPRODUCIBLE` | The union of four sweeps at different seeds. `pnpm stats --chain 56` in `apps/prober` prints it. This is the largest denominator we have and the one we prefer to quote. |
| The 19 sit behind **7 distinct hosts**. | `MEASURED` | `api.bortagent.xyz`, `app.singularry.org`, `clawdmint-api.vercel.app`, `clipx.app`, `x402.quickintel.io`, and two `bubbleupdappos.workers.dev` hosts. |
